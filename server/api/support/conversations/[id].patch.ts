...
export default defineEventHandler(async (event) => {
  ...
  if (body?.read === true) {
    try {
      const conversation = await getCustomerConversationById(
        id,
        Number(user.id)
      );
      ...
      const payload = {
        ...
        lastReadMessageId:
          Number(
            latestMessage?.id || conversation.customerLastReadMessageId || 0
          ) || null,
      };
  ...
  throw createError({ statusCode: 400, message: "Không có thao tác hợp lệ" });
});
