<script setup lang="ts">
  ...
  useSeoHead({
    title: "Hỗ trợ khách hàng | Shop",
    description: "Liên hệ với đội hỗ trợ của chúng tôi 24/7",
  });
  ...
  const pendingUsername = computed(
    () => String(route.query.username || "").trim() || ""
  );

  const showPendingConversation = computed(
    () => !selectedConversation.value && !!pendingUsername.value
  );
  ...
  const getConversationQueryUsername = (
    conversation?: Partial<ConversationItem> | null
  ) => {
    const username = String(conversation?.admin?.username || "").trim();
    return username || null;
  };

  const findConversationByQueryUsername = (username?: string | null) => {
  ...
  const selectConversation = async (conversationId: number) => {
    ...
    const conversation =
      conversations.value.find((item) => item.id === conversationId) || null;
    const username = getConversationQueryUsername(conversation);
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await $fetch<{
        ...
        `/api/support/conversations/${selectedConversationId.value || 0}/messages`,
        {
          ...
          body: {
            content,
            subject:
              selectedConversationId.value || !subjectInput.value.trim()
                ? undefined
      ...
      if (nextConversationId) {
        ...
        const nextConversationUsername =
          getConversationQueryUsername(nextConversation);
        ...
        await router.replace({
          query: {
            ...route.query,
            username:
              nextConversationUsername || pendingUsername.value || undefined,
          },
        });
  ...
  watch(
    ...
    async (username) => {
      ...
      const matchedConversation = findConversationByQueryUsername(
        String(username || "")
      );
      ...
      if (selectedConversationId.value !== matchedConversation.id) {
        await selectConversation(matchedConversation.id);
      }
    }
  );
  ...
  watch(selectedConversation, (conversation) => {
    if (!conversation) {
      useHead({ title: "Hỗ trợ khách hàng" });
      return;
    }
  ...
  watch(showPendingConversation, (visible) => {
    if (!visible || selectedConversation.value) {
      return;
    }

    useHead({
      title: `Chat với ${pendingUsername.value}`,
    });
  });
  ...
  onMounted(async () => {
    ...
    const usernameFromQuery = String(route.query.username || "").trim();
    const matchedConversation =
      findConversationByQueryUsername(usernameFromQuery);
        ...
        <div v-if="!loggedIn" class="px-5 py-16 lg:px-8">
          ...
          >
            ...
            <h2 class="text-2xl font-black text-slate-900 dark:text-white">
              Đăng nhập để mở trung tâm hỗ trợ
            </h2>
            <p
        ...
        <div v-else class="flex min-h-0 flex-1 overflow-hidden">
          <aside
            :class="[
              ...
              selectedConversation || showPendingConversation
                ? 'hidden lg:flex'
                : 'flex',
            ...
            <div class="scrollbar min-h-0 flex-1 overflow-y-auto p-2">
              ...
              <template v-else>
                ...
                >
                  <div class="flex items-start gap-2.5">
                    <div class="relative flex-shrink-0">
                      ...
                      >
                        <img
                          :src="`https://api.dicebear.com/7.x/notionists/svg?seed=${encodeURIComponent(getConversationDisplayUsername(conversation, selectedConversationId === conversation.id ? pendingUsername : null))}`"
                          :alt="
                            getConversationDisplayUsername(
                              conversation,
                              selectedConversationId === conversation.id
                                ? pendingUsername
                                : null
                ...
                >
                  ...
                  >
                    Tạo cuộc trò chuyện mới để liên hệ với đội hỗ trợ.
                  </p>
                </div>
              </template>
            </div>
              ...
              <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="flex min-w-0 items-center gap-3">
                  ...
                  <div class="relative">
                    ...
                    >
                      <img
                        :src="`https://api.dicebear.com/7.x/notionists/svg?seed=${encodeURIComponent(getConversationDisplayUsername(selectedConversation, pendingUsername))}`"
                        :alt="
                          getConversationDisplayUsername(
          ...
          <section
            v-else-if="showPendingConversation"
            class="flex min-h-0 flex-1 flex-col bg-white dark:bg-slate-900"
          >
            <div
              class="border-b border-slate-200 bg-slate-50 px-4 py-3 dark:border-slate-800 dark:bg-slate-800/50 sm:px-6 sm:py-4"
              ...
              <div class="flex flex-wrap items-center justify-between gap-3">
                <div class="flex min-w-0 items-center gap-3">
                  ...
                  <div class="relative">
                    ...
                    >
                      <img
                        :src="`https://api.dicebear.com/7.x/notionists/svg?seed=${encodeURIComponent(getConversationDisplayUsername(null, pendingUsername))}`"
                        :alt="
                          getConversationDisplayUsername(null, pendingUsername)
                        "
                        class="h-full w-full object-cover"
                      />
                    </div>
                ...
                <div class="flex flex-wrap items-center justify-end gap-2">
                  ...
                  >
                    {{ getConversationStatusLabel("new") }}
                  </span>
                </div>
              </div>
            </div>
            ...
            >
              ...
              >
                Gửi tin nhắn đầu tiên để hệ thống tạo cuộc trò chuyện hỗ trợ.
              </p>
            </div>
