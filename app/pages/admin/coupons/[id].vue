<script setup lang="ts">
  ...
  useHead({ title: `Chỉnh sửa mã giảm giá #${id} - Admin Panel` });
  ...
  if (!canManageCoupons.value) {
    toast.error("Từ chối", "Bạn không có quyền chỉnh sửa mã giảm giá");
    await navigateTo("/admin/coupons");
  }

  if (!id) {
    toast.error("Lỗi", "ID coupon không hợp lệ");
    await navigateTo("/admin/coupons");
  }
  ...
  const { data: couponData } = await useFetch<CouponDetail>(
  ...
  watchEffect(() => {
    if (!couponData.value || initialized.value) return;

    form.code = String(couponData.value.code || "");
    form.discountType =
      couponData.value.discountType === "fixed" ? "fixed" : "percent";
    form.discountValue = Number(couponData.value.discountValue || 0);
    form.maxDiscount = Number(couponData.value.maxDiscount || 0);
    form.minOrderValue = Number(couponData.value.minOrderValue || 0);
    form.applicableProductIds = Array.isArray(
      couponData.value.applicableProductIds
    )
      ? couponData.value.applicableProductIds.map((value) => Number(value))
      : [];
    form.usageLimit = Number(couponData.value.usageLimit || 0);
    form.maxPerUser = Number(couponData.value.maxPerUser || 0);
    form.startAt = toBusinessDateTimeLocalValue(couponData.value.startAt);
    form.expiryDate = toBusinessDateTimeLocalValue(couponData.value.expiryDate);
    form.description = String(couponData.value.description || "");
    form.internalNote = String(couponData.value.internalNote || "");
    ...
    try {
      await $fetch(`/api/admin/coupons/${id}`, {
        ...
        body: {
          ...
          status: couponData.value?.status || "active",
          description: form.description || null,
          internalNote: form.internalNote || null,
        },
      });

      toast.success("Thành công", `Đã cập nhật mã ${normalizedCode.value}.`);
      router.push("/admin/coupons");
    } catch (error: any) {
      toast.error(
        "Lỗi",
        error.data?.message || "Không thể cập nhật mã giảm giá"
      );
    } finally {
