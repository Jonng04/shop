...
useHead({ title: "Thêm gói sản phẩm mới - Admin Panel" });
...
if (!canManagePlans.value) {
  toast.error("Từ chối", "Bạn không có quyền tạo gói sản phẩm");
...
// Auto-generate slug từ tên (chỉ khi chưa tự nhập slug)
...
watch(() => form.name, (val) => {
  if (!slugEdited.value) form.slug = toSlug(val);
});
  ...
  try {
    ...
    await $fetch("/api/admin/plans", {
      ...
      body: {
        ...
        name: form.name,
        slug: form.slug || undefined,  // server sẽ tự generate nếu rỗng
        price: form.price,
    ...
    toast.success("Thành công", `Gói "${form.name}" đã được tạo.`);
          ...
          <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div class="col-span-2 space-y-1.5">
              <label class="text-[11px] font-semibold uppercase tracking-widest text-slate-400">
                Slug <span class="font-normal normal-case text-slate-400">(tự động tạo từ tên — có thể sửa)</span>
              </label>
