<script setup lang="ts">
  ...
  useHead({ title: `Sua Flash Sale #${id} - Admin Panel` });

  const toast = useToast();
  const router = useRouter();

  const { data: flashSaleData } = await useFetch<any>(
  ...
  watchEffect(() => {
    ...
    form.name = flashSaleData.value.name || "";
    form.discountType = flashSaleData.value.discountType || "percent";
    form.discountValue = Number(flashSaleData.value.discountValue || 0);
    form.maxDiscount = Number(flashSaleData.value.maxDiscount || 0);
    form.quantityLimit = Number(flashSaleData.value.quantityLimit || 0);
    form.maxPerUser = Number(flashSaleData.value.maxPerUser || 1);
    form.startAt = toBusinessDateTimeLocalValue(flashSaleData.value.startAt);
    form.endAt = toBusinessDateTimeLocalValue(flashSaleData.value.endAt);
    form.note = flashSaleData.value.note || "";
  });

  watch(
    [flashSaleData, plans],
    ([flashSale, availablePlans]) => {
      if (!flashSale || !availablePlans.length) {
        return;
      }

      const nextExpanded = new Set<number>();
      const nextIds = new Set<number>();
      const rawItems = Array.isArray(flashSale.items) ? flashSale.items : [];
      const items =
        rawItems.length > 0
          ? rawItems
          : [
              {
                productId: flashSale.productId || null,
                planId: flashSale.planId || null,
              },
            ];

      items.forEach((item: any) => {
        const productId = Number(item?.productId || 0);
        const planId = item?.planId ? Number(item.planId) : null;
        ...
        availablePlans
          .filter((plan) => Number(plan.productId) === productId)
          .forEach((plan) => nextIds.add(Number(plan.id)));
      });

      expandedProductIds.value = nextExpanded;
      selectedPlanIds.value = [...nextIds];
    },
    { immediate: true }
  );
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      toast.success("Thành công", "Đã cập nhật flash sale");
    ...
    >
      <div class="flex items-center gap-3">
        ...
        <div>
          ...
          >
            Cập nhật cấu hình giảm giá, thời gian và giới hạn bán
          </p>
        </div>
      </div>
      ...
      <div class="flex items-center gap-2">
        ...
        <UiButton class="px-7" :loading="isSubmitting" @click="handleUpdate">
          Lưu cập nhật
        </UiButton>
      </div>
    </div>
        ...
        >
          ...
          <div class="space-y-2">
            ...
            <p class="text-xs text-slate-400">
              ...
              chọn/bỏ chọn toàn bộ. Click vào từng gói để chọn riêng lẻ.
            </p>
          </div>
